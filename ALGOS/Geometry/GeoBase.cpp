...
struct Polygon{
	...
	bool SegmentCrossIn(Point s, Point t){ // s-t cross truely in polygon
		vector<pair<double, Point> > vp; vp.push_back(make_pair(0, s)); vp.push_back(make_pair((t - s).Len(), t));
		for(int i = 0; i < v.size(); i++){
			int ni = (i + 1) % v.size();
			if(IntersectionSegmentSegment(s, t, v[i], v[ni], 0)) return true;
			if(CountTwoLines(s, t - s, v[i], v[ni] - v[i]) == 1 && IntersectionSegmentSegment(s, t, v[i], v[ni])){
				Point c = IntersectionLineLine(s, t - s, v[i], v[ni] - v[i]);
				vp.push_back(make_pair((c - s).Len(), c));
			}
		}
		sort(vp.begin(), vp.end());
		vp.erase(unique(vp.begin(), vp.end()), vp.end());
		for(int i = 1; i < vp.size(); i++){
			Point p = (vp[i].second + vp[i - 1].second) / 2;
			if(Contain(p) == 1) return true; 
		}
		return false;
	}
	/*
